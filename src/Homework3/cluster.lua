...
local b4 = {};
for k, v in pairs(_ENV) do b4[k] = v end -- lua trivia (used to find rogue locals)
...
function obj(s, t, new) --> t; create a klass and a constructor
  function new(_, ...) id = id + 1;
  local i = setmetatable({ a = s, id = id }, t);
  t.new(i, ...); return i end

  t = {};
  t.__index = t;
  return setmetatable(t, { __call = new })
end
...
function SYM.div(i, x, fun, e) --> n; return the entropy
  ...
  e = 0;
  for _, n in pairs(i.has) do e = e + fun(n / i.n) end
  return -e
end
...
function NUM.norm(i, n)
  return n == "?" and n or (n - i.lo) / (i.hi - i.lo + 1E-32)
end
...
function DATA.around(i, row1, rows, cols) --> t; sort other `rows` by distance to `row`
  return sort(map(rows or i.rows,
    function(row2) return { row = row2, dist = i:dist(row1, row2, cols) } end), lt "dist")
end
...
function DATA.cluster(i, rows, min, cols, above) --> t; returns `rows`, recursively halved
  ...
  if #rows > 2 * min then
    ...
    node.right                            = i:cluster(right, min, cols, node.B)
  end
  return node
end
...
function DATA.sway(i, rows, min, cols, above) --> t; returns best half, recursively
  ...
  if #rows > 2 * min then
    ...
    node.left = i:sway(left, min, cols, node.A)
  end
  return node
end
...
function map(t, fun, u) --> t; map a function `fun`(v) over list (skip nil results)
  u = {};
  for k, v in pairs(t) do v, k = fun(v);
  u[k or (1 + #u)] = v
  end
  return u
end
...
function kap(t, fun, u) --> t; map function `fun`(k,v) over list (skip nil results)
  u = {};
  for k, v in pairs(t) do v, k = fun(k, v);
  u[k or (1 + #u)] = v;
  end
  return u
end
...
function lt(x) --> fun;  return a function that sorts ascending on `x`
  return function(a, b) return a[x] < b[x] end
end
...
function many(t, n, u) --> t1; returns some items from `t`
  u = {};
  for i = 1, n do u[1 + #u] = any(t) end
  return u
end

-- ### Strings
function fmt(sControl, ...) --> str; emulate printf
  return string.format(sControl, ...)
end
...
function o(t, isKeys, fun) --> s; convert `t` to a string. sort named keys.
  ...
  return "{" .. table.concat(#t > 0 and not isKeys and map(t, o) or sort(kap(t, fun)), " ") .. "}"
end
...
function main(options, help, funs, saved, fails) --> nil; main program
  saved, fails = {}, 0
  for k, v in pairs(cli(settings(help))) do options[k] = v;
  saved[k] = v
  end
  ...
  os.exit(fails)
end -- returns failure count. and count==0 means "success!"
...
eg("around", "sorting nearest neighbors", function(data)
  ...
  for n, t in pairs(data:around(data.rows[1])) do
    if n % 50 == 0 then print(n, rnd(t.dist, 2), o(t.row.cells)) end
  end
end)
