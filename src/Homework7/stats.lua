...
local function samples(t, n, u)
    u = {};
    for i = 1, n or #t do u[i] = t[math.random(#t)] end
    ;
    return u
end
...
local function cliffsDelta(ns1, ns2) --> bool; true if different by a trivial amount
    ...
    return math.abs(lt - gt) / n <= the.cliff
end
...
local function bootstrap(y0, z0)
    ...
    -- y contains just y0
    -- z contains just z0
    for _, y1 in pairs(y0) do
        add(x, y1);
        add(y, y1)
    end
    for _, z1 in pairs(z0) do
        add(x, z1);
        add(z, z1)
    end
    ...
    for _ = 1, the.bootstrap do
        ...
        if delta(NUM(samples(yhat)), NUM(samples(zhat))) > tobs then n = n + 1 end
    end
...
function RX(t, s)
    table.sort(t)
    return { name = s or "", rank = 0, n = #t, show = "", has = t }
end

function mid(t, n)
    t = t.has and t.has or t
    local n = #t // 2
    return #t % 2 == 0 and (t[n] + t[n + 1]) / 2 or t[n + 1]
end

function div(t)
    t = t.has and t.has or t
    return (t[#t * 9 // 10] - t[#t * 1 // 10]) / 2.56
end

function merge(rx1, rx2, rx3)
    rx3 = RX({}, rx1.name)
    for _, t in pairs { rx1.has, rx2.has } do
        for _, x in pairs(t) do rx3.has[1 + #rx3.has] = x end
    end
    table.sort(rx3.has)
    rx3.n = #rx3.has
    return rx3
...
function tiles(rxs) --> ss; makes on string per treatment showing rank, distribution, and values
    ...
    for _, rx in pairs(rxs) do
        lo, hi = min(lo, rx.has[1]), max(hi, rx.has[#rx.has])
    end
    for _, rx in pairs(rxs) do
        local t, u = rx.has, {}
...
eg.sample = function()
    for i = 1, 10 do
        print("", table.concat(samples { "a", "b", "c", "d", "e" }))
    end
end
...
eg.pre = function()
    ...
    for i = 1, 10 do
        local t1, t2 = {}, {}
        for j = 1, 32 do
            t1[1 + #t1] = gaussian(10, 1);
            t2[1 + #t2] = gaussian(d * 10, 1)
        end
